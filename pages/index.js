...
export default function Dashboard() {
  ...
  const [visionGerencialData, setVisionGerencialData] = useState([]);
  ...
  const cargarDatos = useCallback(async () => {
    ...
    try {
      ...
      setVisionGerencialData(Array.isArray(visionGerencial) ? visionGerencial : []);
      setColaboradores(Array.isArray(cols) ? cols : []);

      // ANÁLISIS IA AVANZADO: Detectar patrones en quejas
      if (Array.isArray(satisfaccion) && satisfaccion.length > 0) {
        const analisis = {};
        
        // Patrones por sucursal
        const sucursales = {};
        satisfaccion.forEach(q => {
          if (!sucursales[q.sucursal]) sucursales[q.sucursal] = { total: 0, abiertas: 0 };
          sucursales[q.sucursal].total++;
          if (q.status !== 'CERRADA') sucursales[q.sucursal].abiertas++;
        });
        
        // Detectar sucursales con problemas
        Object.entries(sucursales).forEach(([suc, data]) => {
          if (data.abiertas / data.total > 0.3) {
            analisis[`alerta-${suc}`] = {
              nivel: 'ALTO',
              mensaje: `${suc} tiene ${data.abiertas} quejas abiertas (${Math.round(data.abiertas/data.total*100)}% del total)`,
              sugerencia: 'Priorizar revisión de procesos en esta sucursal'
            };
          }
        });
        ...
        const motivos = {};
        satisfaccion.forEach(q => {
          motivos[q.motivo] = (motivos[q.motivo] || 0) + 1;
        });
        const topMotivos = Object.entries(motivos)
          .sort((a, b) => b[1] - a[1])
          .slice(0, 3);
        
        analisis.topMotivos = topMotivos.map(([motivo, count]) => ({
          motivo,
          count,
          sugerencia: `Principal causa de quejas: ${count} casos. Revisar procedimiento asociado.`
        }));
  ...
  // MÉTRICAS PARA SATISFACCIÓN
  const metricasSatisfaccion = {
    npsPromedio: visionGerencialData.find(v => v.indicador === 'NPS Promedio (Satisfacción)')?.enero || '8.5',
    felicitaciones: visionGerencialData.find(v => v.indicador === '% Felicitaciones Clientes')?.enero || '92',
    quejasAbiertas: satisfaccionFiltrada.filter(q => q.status !== 'CERRADA').length,
    tiempoPromedioCierre: Math.round(
      satisfaccionFiltrada
        .filter(q => q.tiempoCierre)
        .reduce((acc, q) => acc + q.tiempoCierre, 0) / 
      (satisfaccionFiltrada.filter(q => q.tiempoCierre).length || 1)
    )
  };
                        ...
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <InfoTooltip content={
                            <div className="w-56 text-left space-y-1">
                              <p><span className="text-emerald-400 font-bold">🟢 Excelente:</span> FTR ≥98%, Tiempo ≤7min y NoConf=0</p>
                              <p><span className="text-amber-400 font-bold">🟡 En desarrollo:</span> FTR ≥95% o Tiempo ≤8min o NoConf≤2</p>
                      ...
                      <tr key={idx} className="border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="p-4 text-center">
                          <span className={`font-mono font-bold ${
                            col.ftr >= 98 ? 'text-emerald-600' : 
                            col.ftr >= 95 ? 'text-amber-600' : 'text-red-600'
                          }`}>
                            {col.ftr.toFixed(1)}%
                          </span>
                        </td>
          ...
          <div className="space-y-6">
              ...
              <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-3xl font-black text-slate-800 mt-1">{metricasSatisfaccion.npsPromedio}</p>
                  </div>
                  <span className="text-xs text-slate-400">Meta: 9.0</span>
                </div>
                <div className="mt-2 h-1 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-1 bg-blue-600 rounded-full" style={{ width: `${(parseFloat(metricasSatisfaccion.npsPromedio)/9)*100}%` }}></div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-3xl font-black text-emerald-600 mt-1">{metricasSatisfaccion.felicitaciones}%</p>
                  </div>
                  <span className="text-xs text-slate-400">Meta: 90%</span>
                </div>
                <div className="mt-2 h-1 w-full bg-slate-100 rounded-full overflow-hidden">
                  <div className="h-1 bg-emerald-600 rounded-full" style={{ width: `${(metricasSatisfaccion.felicitaciones/90)*100}%` }}></div>
                </div>
              </div>

              <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm">
                <p className="text-xs font-bold text-slate-400 uppercase">Quejas Abiertas</p>
                <p className="text-3xl font-black text-amber-600 mt-1">{metricasSatisfaccion.quejasAbiertas}</p>
                <p className="text-xs text-slate-400 mt-2">Total: {satisfaccionFiltrada.length}</p>
              </div>

              <div className="bg-white rounded-xl p-5 border border-slate-100 shadow-sm">
                <p className="text-xs font-bold text-slate-400 uppercase">Tiempo Prom. Cierre</p>
                <p className="text-3xl font-black text-slate-800 mt-1">{metricasSatisfaccion.tiempoPromedioCierre} hrs</p>
                <p className="text-xs text-slate-400 mt-2">Meta: 48 hrs</p>
              </div>
            </div>
            ...
            {Object.keys(analisisIA).length > 0 && (
              <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-2xl p-5 border border-purple-100">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <h3 className="text-sm font-bold text-purple-800">Análisis Inteligente de Patrones</h3>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {Object.entries(analisisIA).map(([key, value]) => {
                    if (key === 'topMotivos') return null;
                    return (
                      <div key={key} className="bg-white/80 rounded-lg p-3 border border-purple-200">
                        <p className="text-xs font-bold text-purple-600 uppercase">{value.nivel}</p>
                        <p className="text-sm mt-1">{value.mensaje}</p>
                        <p className="text-xs text-purple-600 mt-1">💡 {value.sugerencia}</p>
                      </div>
                    );
                  })}
                </div>
                {analisisIA.topMotivos && (
                  <div className="mt-4 bg-white/80 rounded-lg p-3 border border-purple-200">
                    <p className="text-xs font-bold text-purple-600 uppercase mb-2">Principales causas</p>
                    <div className="space-y-2">
                      {analisisIA.topMotivos.map((item, idx) => (
                        <div key={idx} className="flex items-center justify-between">
                          <span className="text-sm">{item.motivo}</span>
                          <span className="text-xs font-bold text-purple-600">{item.count} casos</span>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-4">
              <h3 className="text-sm font-bold text-slate-800 mb-3 flex items-center gap-2">
                🔍 Filtrar quejas
                <InfoTooltip content="Filtra por sucursal, proceso, estado, mes o semana" />
              </h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                <select 
                  ...
                  className="text-xs border rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-[#0066CC]"
                >
                                ...
                                <div key={i} className="bg-white rounded-lg border border-slate-200 p-3">
                                  <div className="flex justify-between items-center mb-1">
                                    <span className="text-[10px] font-bold text-slate-500 uppercase">
                                      📋 Sesión {new Date(seg.fechaRegistro).toLocaleDateString()}
                                    </span>
