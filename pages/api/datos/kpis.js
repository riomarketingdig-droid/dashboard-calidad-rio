...
export default async function handler(req, res) {
  try {
    ...
    satisfaccion.forEach(row => {
      const nps = parseFloat(row[22]) || 0; // NPS en columna W (índice 22)
      const felicitacion = row[10]?.toLowerCase().includes('felicitacion') ? 1 : 0;
      const status = row[20] || '';
      const fechaRecepcion = row[4];
      const fechaCierre = row[21];
      if (nps > 0) totalNPS += nps;
      if (felicitacion) totalFelicitaciones++;
      if (status !== 'CERRADA') quejasAbiertas++;
      if (fechaCierre && fechaRecepcion) {
        const dias = Math.round((new Date(fechaCierre) - new Date(fechaRecepcion)) / (1000 * 60 * 60 * 24));
        totalTiempoCierre += dias;
        countCerradas++;
      }
    });
    const promNPS = satisfaccion.length ? (totalNPS / satisfaccion.length).toFixed(1) : 'N/A';
